...
public class Worker {
    ...
    private static void doWork(String message) {
        ...
        Elements body = dataArticle.setBody(document, listBody, "", replaceURL, WorkerMedia.folder);
...
//        listBody.add("#viewdoc > div.box > div.doc-preview");
    ...
    private static void uploadSrc(String source, List<String> srcList) {
        ...
        try {
            ...
            for (String aSrcList : srcList) {
                try {
                    logger.info(new Date().toString() + aSrcList);
                    channelSrc.basicPublish("", SRC_QUEUE_NAME,
                            null,
                            (source + "#" + aSrcList).getBytes());
    ...
    private static void uploadArticle(String source, DataArticle dataArticle) {
        ...
        try {
            ...
            try {
                String message = dataArticle.getArticle().toString();
                logger.info(new Date().toString()+" sent article");
                channelArticle.basicPublish("", ARTICLE_QUEUE_NAME,
                        null,
                        (source + "#" + message).getBytes());
            } catch (UnsupportedEncodingException e) {
                logger.error(e.getMessage());
            }
